...
function normalizeModelConfig(input = {}) {
  ...
  for (const key of MODEL_KEYS) assertSupportedModel(key, next[key])
  return next
}

async function readModelConfig(token) {
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const { config } = await readModelConfig(token)
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      await writeModelConfig(token, next, sha, commitMessage || 'chore: update analysis model config')
